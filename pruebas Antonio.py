...
cursos = [{"codigo": "123","id":"2016108660","creditos":"2", "nombre": "Discreta", "tottareas": 0,
           "tareas":[{'porcentaje': 2.0, 'nombre': 'tarea1'},{'porcentaje': 3.0, 'nombre': 'tarea2'}], "totexaemenes": 0, "examenes": [{'examen1':20.0}],
           "totproyectos": 0, "proyectos": [{'proyecto1':25.0}], "totquiz": 0, "quiz": [], "tototros": 0, "otros": [],
           "totlaboratorios": 0, "laboratorios": [],"estudiantes mat":[{"nombre": "Carlos José Mora", "correo": "cjmora@hotmail.com", "pass": "cjmora", "id": "2010356721", "tipo": "1",
     "carrera": "3", "telefono": "40224312","notas":[{"nombre":"tarea1","nota":50},{"nombre":"tarea2","nota":80}]}]},
    ...
    for t in cursos:
        for w in t['estudiantes mat']:  # Ciclo que recorre la lista de cursos, y los diccionarios dentro de cada curso hasta encontrar si el estudiante esta matriculado en un curso, en ese caso, lo imprime inmediatamente.
            if xl in w['nombre']:
                ...
                break
    z=input("""Digite una opcion, por favor
        1.  Ver Notas
        2.  Regresar al menu de estudiantes
        """)
    if z=="1":
        for o in w["notas"]:
            print(o["nombre"],"\t""\t""\t",o['nota'],"\n")
    otra_accion_est(xl)
...
def edit_evaluations():
    ...
    for i in cursos:
        if code_course == i["codigo"]:
            ...
            6.Otro
            """)
            resp = input("Digite la opción de la evaluación que desea modificar. ")
            if resp == "1":
                posicion = "tareas"
            elif resp == "2":
                posicion = "proyectos"
...
def notaes(xl):
    x=input("""Digite el codigo de curso donde sea agregar una nota.
    """)
    for i in cursos:
        if x ==i["codigo"]:  # recorro la lista de cursos en busca del codigo del curso
            for q in i["estudiantes mat"]:  ##Ciclo que busca todos los estudiantes matriculados en un curso y los imprime.
                    print(q['nombre'], "\t", "\t", "\t", q['id'], "\n")
            est=input("Digite la identificacion del estudiante al cual desea agregar una nota")
            nomnot=input("Digite el nombre de la evaluacion por asignar")
            notnot=int(input("Digite la nota obtenida por el estudiante"))
            b=[]
            r={}
            r["nombre"]=nomnot
            r["nota"]=notnot
            ...
            print(cursos)
            menu_evaluaciones(xl)
    print("El codigo que ingreso no se encuentra en el sistema, intente de nuevo")
    notaes(xl)

def modinot():
    x=input("""Digite el codigo de curso donde sea modificar una nota.
    """)
    for i in cursos:
        if x ==i["codigo"]:  # recorro la lista de cursos en busca del codigo del curso
            for q in i["estudiantes mat"]:  ##Ciclo que busca todos los estudiantes matriculados en un curso y los imprime.
                    print(q['nombre'], "\t", "\t", "\t", q['id'], "\n")
            est=input("Digite la identificacion del estudiante al cual desea modificar una nota")
            nomnot=input("Digite el nombre de la evaluacion por modificar")
            notnot=int(input("Digite la nota obtenida por el estudiante"))
            if est==q['id']:
                for t in q['notas']:
                    if nomnot==t["nombre"]:
